        ...
        @endif
              ...
              <table class="table table-responsive table-advance table-hover">
                ...
                @foreach ($anneescolaire as $anneescolaires)                         
                  <tr>                 
                    <td></td>
                    <td>{{ $anneescolaires->lib_anneescolaire }}</td>                                                   
                        <td><a href="{{ route('anneescolaire.edit',$anneescolaires->id) }}"  class="btn btn-success"><i class="glyphicon glyphicon-edit"></i></a></td>
                        <td>
                          <form  method="POST" action="{{ route('anneescolaire.destroy',$anneescolaires->id) }}" >
                            @csrf
                            @method('DELETE')
              ...
              <div class="modal-dialog">
                <div class="modal-content">
                  <div class="modal-header">
                    ...
                    <h4 class="modal-title">Ajouter une Année-Scolaire</h4>
                  </div>
                  <div class="modal-body">

                    <form role="form" method="POST" action="{{ route('anneescolaire.store') }}">
                          @csrf
                      <div class="form-group">
                        <label for="exampleInputlibelle">Libellé</label>
                        <input type="text" class="form-control"  name="lib_anneescolaire" id="exampleInputlibelle" placeholder="Entrer une Annee Scolaire">
                      </div>                   
