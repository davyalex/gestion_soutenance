...
<div class="col-md-6  panel-body">           
    <form role="form" method="POST" action="{{ route('salle.update', $salles->id) }}">
        @csrf
        @method('PUT')

        @if (session()->has('update_message'))
        <div class="alert alert-success" role="alert">{{ session('update_message') }} <a href="{{ route('list_salle') }}" class="btn btn-default"> <i class="glyphicon glyphicon-eye-open"></i> Consulter la liste</a></div>

        @else
        <div class="alert alert-warning">
            Vous êtes sur le point de modifier filière  <strong>{{ $salles->lib_salle }}</strong>       
        </div>
        @endif
    ...
    <div class="form-group">
      ...
      <input type="text" class="form-control"  name="lib_salle" value="{{ $salles->lib_salle }}">
    </div>                   
